...
<style>
    ...
    /* Box hiển thị SBD (Kích thước y hệt 1 nút bấm) */
    .sbd-box {
        text-align: center; 
        color: #1E88E5; 
        background-color: #E3F2FD; 
        height: 50px;
        line-height: 46px; /* Căn giữa chữ theo chiều dọc */
        border-radius: 8px;
        font-size: 24px;
        font-weight: bold;
...
with tab1:
    ...
    if not st.session_state.nhap_xong_sbd:
        st.markdown("<p style='text-align:center; font-weight:bold; margin-bottom: 2px;'>Nhập Số báo danh:</p>", unsafe_allow_html=True)
        
        sbd_display = st.session_state.sbd_val if st.session_state.sbd_val else "---"
        
        # Đặt ô hiển thị vào đúng cột giữa để nó bằng chiều rộng của 1 nút bấm
        r0c1, r0c2, r0c3 = st.columns(3)
        with r0c2:
            st.markdown(f"<div class='sbd-box'>{sbd_display}</div>", unsafe_allow_html=True)
